...
export default async function VehicleDetails({params}){
  ...
  return <><Header/><main className="main adminPage">
    <section className="adminHero"><div><span className="badge">Автомобиль</span><h1>{vehicleTitle(vehicle)}</h1><p className="muted">{customer?nameOf(customer):'без клиента'} · {history.length} записей обслуживания · {leads.length} заявок</p></div>{customer?<Link className="btn primary" href={`/admin/customers/${customer.id}`}>К клиенту</Link>:<Link className="btn primary" href="/admin">Назад</Link>}</section>
    ...
    <Block title="Действия с автомобилем"><VehicleEditForm vehicle={vehicle}/></Block>
    <Block title="Добавить обслуживание"><ServiceHistoryForm vehicleId={vehicle.id}/></Block>
