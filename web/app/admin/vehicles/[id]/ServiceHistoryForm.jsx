...
const emptyForm={service_date:'',title:'',description:'',mileage:'',price:'',comment:'',lead_id:''};

function leadLabel(lead){return [lead.public_id||'Заявка',lead.type,lead.request_text].filter(Boolean).join(' · ').slice(0,140)}

export default function ServiceHistoryForm({vehicleId,leads=[]}){
  ...
  async function submit(e){
    ...
    try{
      ...
      const r=await fetch(`/api/admin/vehicles/${vehicleId}/service-history`,{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify(payload)});
