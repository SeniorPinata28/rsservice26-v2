...
function nextAction(lead){
 if(lead.status==='completed')return 'Заявка выполнена';
 if(!lead.customer_id)return 'Нужно подтвердить клиента';
 if(!lead.vehicle_id)return 'Нужно привязать автомобиль';
 return 'Можно вести обслуживание';
}
...
export default function AdminFilters({leads}){
 ...
 const filtered=useMemo(()=>clean.filter(lead=>{
  ...
  if(q){
   const hay=[lead.public_id,lead.name,lead.phone,lead.vin,lead.car_text,lead.request_text,lead.type,lead.status].map(v=>String(v||'').toLowerCase()).join(' ');
 ...
 return <section className="card adminList" style={{padding:16}}>
   ...
   <div style={{display:'grid',gridTemplateColumns:'repeat(auto-fit,minmax(220px,1fr))',gap:10,marginBottom:14}}>
    <select className="input" value={status} onChange={e=>setStatus(e.target.value)}><option value="all">Все статусы</option>{Object.entries(leadStatuses).map(([k,v])=><option key={k} value={k}>{v}</option>)}</select>
    <input className="input" value={query} onChange={e=>setQuery(e.target.value)} placeholder="Поиск: номер, телефон, имя, VIN, автомобиль, текст"/>
   </div>
   ...
   <div style={{display:'grid',gap:10}}>
    {filtered.map(lead=><article key={lead.id} className="card" style={{padding:14,borderRadius:18,boxShadow:'0 8px 22px rgba(15,23,42,.055)'}}>
      <div style={{display:'grid',gridTemplateColumns:'1fr',gap:10,alignItems:'start'}}>
        ...
        <div className="notice" style={{margin:0,fontWeight:900}}>{nextAction(lead)}</div>
      </div>
      <p style={{margin:'10px 0 0',lineHeight:1.45}}>{shortText(lead.request_text||'Без текста')}</p>
      <div style={{display:'flex',gap:8,flexWrap:'wrap',marginTop:10}}><Link className="btn primary" href={`/admin/leads/${lead.id}`} style={{marginTop:0}}>Открыть</Link>{lead.customer_id&&<Link className="btn" href={`/admin/customers/${lead.customer_id}`} style={{marginTop:0}}>Клиент</Link>}{lead.vehicle_id&&<Link className="btn" href={`/admin/vehicles/${lead.vehicle_id}`} style={{marginTop:0}}>Авто</Link>}</div>
     </article>)}
   </div>
  </section>
}