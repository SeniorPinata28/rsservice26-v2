...
export default function AvailabilityClient({initialQuery=''}){
  ...
  return <>
    <section className="availabilityHero"><div><span className="eyebrow">Проверка цены и остатков</span><h1>Проверить наличие запчасти</h1><p>Введите артикул или точное название. Если запрос общий, выберите уточнение ниже — так поиск даст больше полезных результатов.</p><div className="miniBenefits"><span>Проверяем наличие у поставщиков</span><span>Согласуем цену до заказа</span><span>Можно сразу записаться на установку</span></div></div></section>
